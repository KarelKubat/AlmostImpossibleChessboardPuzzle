...
So there are 16 positions where you can flip a coin to change the leftmost bit and the second-to-rightmost bit in the checksum: these are the positions 34, 35, 38, 39, 42, 43, 46, 47, and so on. You can choose to change just these two bits (position 34) or you can change other bits too at the same time - it's up to you. But there are 16 positions to choose from, and they cover all possibilities to change or not change the other 4 bits at the same time. That's pretty logical: if those positions flip 2 bits already, then the remaining 4 bits of the 6-bit checksum must be also covered. So you need 2^4 tiles to cover all the possibilities - which is 16.
